...
def simulate_dealer_cards(game_state):
    while True:
        ...
        # draw one card, recompute on next loop
...
def m_loss(game_state):
    game_state['losses'] += 1
    game_state['balance'] -= game_state['bet']
    game_state['bet'] = min(game_state['bet'] * 2, TABLE_MAX, game_state['balance'])
...
def martingale_strategy(game_state):
    ...
    game_state['player_total'], _ = current_total_and_soft(game_state['player_hand']) #only computing the total, no need to know if soft or hard
...
def martingale_run(game_state):
    ...
    for i in range(num_simulations):
        ...
        b0 = game_state['original_balance']
        ...
        session_stake = sum(session_bets)
...
# 95% CI for loss per 100u staked
...
print("Betting rule: Martingale; base bet =", game_state['original_bet'])
...
plt.title(f"Rolling Mean Bet – Martingale ({num_simulations} Sessions)")
plt.xlabel("Session #")
plt.ylabel("Bet Size (£)")
plt.grid(True)
plt.tight_layout()
...
plt.title(f"Final Balance Distribution – Martingale ({num_simulations} Sessions)")
...
plt.title(f"Return Distribution Per Session – Martingale Betting")
plt.grid(alpha=0.3)
plt.tight_layout()
plt.show()